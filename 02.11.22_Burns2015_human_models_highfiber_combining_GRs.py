...
GR_file_list = glob.glob('*V01_Burns2015_human_highfiber_objective.txt')

GR_file_names = []

for elem in GR_file_list:
    sample_name = re.sub("_V01_Burns2015_human_highfiber_objective.txt", "", elem)  
...
GRs =  {}
for elem in GR_file_list:
    sample_name = re.sub("_V01_Burns2015_human_highfiber_objective.txt", "", elem) #delete extra crap on string.
...
with open('Burns2015_human_models_highfiber_GRs_combined.csv', 'w') as f:
    for key in GRs.keys():
        f.write("%s,%s\n"%(key,GRs[key]))
