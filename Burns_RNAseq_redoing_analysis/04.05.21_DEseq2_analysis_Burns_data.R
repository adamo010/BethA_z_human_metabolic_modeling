...
#dds <- DESeqDataSetFromMatrix(countData=countData, 
                             # colData=metaData, 
                              #design = ~Description + Patient_Blind_ID) #this part indicates that we have paired data.

#hmmm. Error in DESeqDataSetFromMatrix(countData = countData, colData = metaData,  : ncol(countData) == nrow(colData) is not TRUE

#may have to set first column of count_data as index?
...
#NOTE HERE: design formula used to be design = ~Description + Patient_Blind_ID; this is (maybe?) wrong
...
colData(dds)
colData(dds2)
#these should be identical, because we're using the same metadata, which is col_data, for both dds and dds2
...
#look at same results with different design formula
res2 <- results(dds_from_deseq2)
head(results(dds_from_deseq2, tidy=TRUE)) #let's look at the results table

summary(res2) #summary of results for differential gene expression
res2 <- res2[order(res2$padj),] #re-order by p-value
head(res2)

#try to see by patient_blind_ID
res_metadata <- mcols(res, use.names=TRUE) #nothing notable
res_metadata
res_metadata2 <- mcols(res2, use.names=TRUE) #nothing notable
res_metadata2
...
#2nd design formula
par(mfrow=c(2,3))
plotCounts(dds_from_deseq2, gene="KDM5D", intgroup="Description")
plotCounts(dds_from_deseq2, gene="RPS4Y1", intgroup="Description")
plotCounts(dds_from_deseq2, gene="UTY", intgroup="Description")
plotCounts(dds_from_deseq2, gene="DDX3Y", intgroup="Description")
plotCounts(dds_from_deseq2, gene="USP9Y", intgroup="Description")
plotCounts(dds_from_deseq2, gene="LBP", intgroup="Description")
...
###########step 7 (added 04.09.21): figure out how to extract the fold change per gene per patient_blind_ID
#possibly impossible
...
#resultsNames(dds_from_deseq) # to see what names to use
#resultsNames(dds_from_deseq2)
...
#############JUNK: for heatmaps only
# Transform count data using the variance stablilizing transform: already done
head(vsdata)

#let's try making vsdata aware of the design formula:
vsdata <- assay(vsdata) 
...
res_sig <- res_df[res_df$Gene %in% sigGenes,] #filter results (res_df) by significantly differentially expressed genes only. 
#There's actually only 261 genes- nice
res_sig <-res_sig %>% 
  arrange(desc(abs(log2FoldChange))) #sort sigGenes by absolute logfold change.
top_host_gene_expression <- res_sig[abs(res_sig$log2FoldChange) > 1.5,] #pull out only genes with a log2fold change greater than 1.5: 217 genes.

#now, filter vsdata_df by the rownames of top_host_gene_expression
genes_for_filtering <- rownames(top_host_gene_expression) #make list of genes to include
filtered_genes_per_sample <- vsdata_df[vsdata_df$Gene %in% genes_for_filtering,] #filter per-sample DF by significant gene
write.csv(as.data.frame(filtered_genes_per_sample), file="04.13.21_DEseq2_sig_results_Burns_data_by_sample_design_fixed.csv")

#this is all very well and good, but... how is fold change being calculated??? I want fold change between tumor and normal!
